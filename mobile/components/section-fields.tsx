/**
 * Fields for one profile section — shared by the per-section editor and the
 * welcome survey. `draft` is that section's value; `set` replaces it.
 */
import { StyleSheet, Text, View, Pressable } from "react-native";
import { colors, radius } from "../lib/theme";
import { a11y, a11yFont } from "../lib/a11y";
import { BigButton, BigField, BodyText } from "./big-ui";
import type { ProfilePerson, SectionKey } from "../lib/v2-types";
...
/** Repeatable rows bound to the `people` array section. */
function RepeatRows({
  draft,
  set,
  fields,
    ...
    <View style={{ gap: a11y.space(4) }}>
      {rows.map((row, i) => (
        <View key={i} style={styles.rowCard}>
          ...
          {fields.map(([k, label]) => (
            <BigField key={k} label={label} value={row[k] ?? ""} onChangeText={(v) => update(i, k, v)} keyboardType={k === "phone" ? "phone-pad" : "default"} />
          ))}
          <BigButton label="Remove" icon="trash" variant="secondary" onPress={() => onChange(rows.filter((_, j) => j !== i))} accessibilityLabel={`Remove ${firstLabel(fields)} ${i + 1}`} />
        </View>
      ))}
      <BigButton
        label={addLabel}
...
function firstLabel(fields: [string, string][]) {
